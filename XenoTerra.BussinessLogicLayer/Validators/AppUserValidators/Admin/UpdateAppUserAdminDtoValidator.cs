...
namespace XenoTerra.BussinessLogicLayer.Validators.AppUserValidators.Admin
{
    public class UpdateAppUserAdminDtoValidator : AbstractValidator<UpdateAppUserAdminDto>
    {
        public UpdateAppUserAdminDtoValidator()
        {
            RuleFor(x => x.Id)
                .NotEqual(Guid.Empty)
            ...
            RuleFor(x => x.FollowersCount)
                .GreaterThanOrEqualTo(0)
                .When(x => x.FollowersCount.HasValue)
                .WithMessage("Followers count cannot be negative.");

            RuleFor(x => x.FollowingCount)
                .GreaterThanOrEqualTo(0)
                .When(x => x.FollowingCount.HasValue)
                .WithMessage("Following count cannot be negative.");

            RuleFor(x => x.LastActive)
                .LessThanOrEqualTo(DateTime.UtcNow)
                .When(x => x.LastActive.HasValue)
                .WithMessage("Last active time cannot be in the future.");
        }
    }
}
