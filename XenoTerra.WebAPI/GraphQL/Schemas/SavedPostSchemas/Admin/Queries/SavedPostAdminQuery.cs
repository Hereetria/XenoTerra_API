...
namespace XenoTerra.WebAPI.GraphQL.Schemas.SavedPostSchemas.Admin.Queries
{
    public class SavedPostAdminQuery(IMapper mapper, IQueryResolverHelper<SavedPost, Guid> queryResolver)
    {
            ...
            IResolverContext context)
        {
            ...
            var connection = ConnectionMapper.MapAdminConnection<SavedPost, ResultSavedPostWithRelationsDto>(
                entityAdminConnection,
                _mapper
            );

            return GraphQLConnectionFactory.Create<SavedPostAdminConnection, ResultSavedPostWithRelationsDto>(connection);
        }
