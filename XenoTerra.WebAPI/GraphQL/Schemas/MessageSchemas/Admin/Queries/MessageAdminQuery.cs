...
namespace XenoTerra.WebAPI.GraphQL.Schemas.MessageSchemas.Admin.Queries
{
    ...
    public class MessageAdminQuery(IMapper mapper, IQueryResolverHelper<Message, Guid> queryResolver)
    {
            ...
            IResolverContext context)
        {
            ...
            var connection = ConnectionMapper.MapConnection<Message, ResultMessageWithRelationsAdminDto>(
                entityAdminConnection,
                _mapper
            );

            return GraphQLConnectionFactory.Create<MessageAdminConnection, ResultMessageWithRelationsAdminDto>(connection);
        }
        ...
        public async Task<ResultMessageWithRelationsAdminDto?> GetMessageByIdAsync(
            string? key,
            [Service] IMessageQueryService service,
            [Service] IMessageResolver resolver,
            IResolverContext context)
        {
            ...
            return entity is null ? null : _mapper.Map<ResultMessageWithRelationsAdminDto>(entity);
        }
    }
}
